...
const Layout = (props) => {
  ...
  const [addressBlockchains, setAddressBlockchains] = useState({});
  ...
  const fetchAddressInfo = async () => {
    ...
    try {
      ...
      if (isAdminPages) {
        ...
        // Custom address nickname, just in case.
        if (res.nickname) setNickName(res.nickname);

        // * case user searched for an address
        const addressChains = res.blockchains;
        setAddressBlockchains(res.blockchains);

        const foundNetworkType = addressChains?.[networkType];

        if (foundNetworkType) {
          dispatch(setNetworkType(networkType));
        } else {
          const availableNetworks = Object.keys(addressChains);
          if (availableNetworks.length === 1) {
            dispatch(setNetworkType(availableNetworks[0]));
          } else {
            dispatch(setNetworkType('all'));
          }
        }

        const availableNetworks = Object.keys(addressChains);

        // Check if only 'all' network is available
        if (availableNetworks.length === 1 && availableNetworks[0] === 'all') {
          setIsOnlyAllNetwork(true);
        }
      }
